properties {
    ...
    $PSBPreference.Build.CompileDirectories = 'Prepend', 'Private', 'Public', 'Append'
...
task UpdateAssemblyVersion -Depends 'SetBuildVariables' {
    $ProjectFile = [IO.Path]::Combine($DotNetSrcFolder ,'OPassCliPS.csproj')
...
task DotNetBuild -Depends 'UpdateAssemblyVersion' {
    ...
    $OutputLibFolder = [IO.Path]::Combine($PSBPreference.Build.ModuleOutDir,'bin')
    ...
    # do {
    #     try {
    #         Update-ModuleManifest -Path $ManifestPath -NestedModules $LibraryList
    #         $Retry = $false
    #     }
    #     catch {
    #         $ManifestPath
...
task CreateReleaseAsset -Depends StageFiles {
    $DestinationZip = Join-Path -Path (Split-Path -Path $env:BHBuildOutput) -ChildPath ('OPassCliPS-v{0}.zip' -f $ModuleVersion)
...
task Build -FromModule PowerShellBuild -depends @(
    'DotNetBuild','CopyLicense','CreateReleaseAsset'
)

task PrepareToPublish -depends @(
    'Build','UpdateChangeLog'
)
